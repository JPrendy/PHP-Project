<!--Search_by_title- brings up the books you asked for by book title and/or author, 
this displays the first 5 books and allows you to check if it is reserved or not.
(1)If the book is reserved it will display a message saying that it is already booked when you click to reserve it.
(2)if the book isn’t registered it will be reserved into the reserve table,
 this says the book you reserved, who reserved it and when you reserved it.-->
...
isset($_POST["Author"])) {
$user3 = $_POST["BookTitle"];
$user4= $_POST["Author"];


 $_SESSION["BookTitle"]="$user3" ;//it's is storing these variables for later
  $_SESSION["Author"]="$user4" ;//it's is storing these variables for later
...
$result = mysql_query("SELECT ISBN,BookTitle, Author, Edition, Year, CategoryID, Reservation FROM books WHERE BookTitle like '%$user3%' and Author like '%$user4%' limit 5")or trigger_error(mysql_error().$sql);
//this is where it selects all the books that meet the requirements


if(mysql_num_rows($result) <=0) 
{echo"<script> alert('we couldnt find the result') </script>";}
...
//this displays all the rows of informaton in the table books
echo "<tr><td>";
echo($row[0]);
echo "</td><td>";
echo($row[1]);
...
$sql = "UPDATE books SET Reservation='Y'  WHERE ISBN='$user2' && Reservation='N'";
...
<!--this links to the other page-->
<a href="http://localhost/WebD1/search_by_title_2.php" >Page:2</a>
